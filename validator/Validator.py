...
def detect_from_db(myip, proxy, proxies_set):
    ...
    if result:
        ...
        if proxy[2] < 1:
            ...
            score = proxy[2]-1
            sqlhelper.update(
                {'ip': proxy[0], 'port': proxy[1]}, {'score': score})
            proxy_str = '%s:%s' % (proxy[0], proxy[1])
            proxies_set.add(proxy_str)
...
def detect_proxy(selfip, proxy, queue2=None):
    ...
    protocol, types, speed = getattr(sys.modules[__name__], config.CHECK_PROXY['function'])(
...
def _checkHttpProxy(selfip, proxies, isHttp=True):
    ...
    try:
        start = time.time()
        r = requests.get(url=test_url, headers=config.get_header(),
...
def baidu_check(selfip, proxies):
    ...
    try:
        start = time.time()
        r = requests.get(url='https://glosbe.com/gapi/tm?from=es&dest=zh&format=json&phrase=estoy&page=5&pretty=true',
                         headers=config.get_header(), timeout=config.TIMEOUT, proxies=proxies)
        r.encoding = chardet.detect(r.content)['encoding']
...
def getMyIP():
    try:
        r = requests.get(url=config.TEST_IP,
                         headers=config.get_header(), timeout=config.TIMEOUT)
